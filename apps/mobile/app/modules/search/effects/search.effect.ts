...
export class SearchEffects {
  ...
  search$ = this.actions$
    ...
    .map(
      (action: SearchActions.SearchAction) =>
        new SearchActions.UpdatedAction({
          searching: true,
          term: action.payload
        })
    )
  ...
  @Effect()
  clear$ = this.actions$.ofType(SearchActions.ActionTypes.CLEAR).map(action => {
  ...
  @Effect()
  cancel$ = this.actions$.ofType(SearchActions.ActionTypes.CANCEL).map(action => {
