...
import { BaseComponent, UserActions, LogService, ModalActions, WindowService, ProgressService, UserState, ICoreState, UIState, UserService, ModalState } from '@ngatl/core';
...
// app
import { sortAlpha, isObject } from '@ngatl/utils';
import { getResolution } from '@ngatl/nativescript';
import { IConferenceAppState } from '../../../ngrx';
import { AppService, AWSService } from '@ngatl/nativescript';
...
export class DashboardComponent extends BaseComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  public startBadge() {
    ...
    this._initBeacon().then(_ => {
      ...
      this._userService.userInitialized$.pipe(
  ...
  ngOnInit() {
    if (!this._ngOnInitFired) {
      ...
      this._store.pipe(select((s: ICoreState) => s.user),
        takeUntil(this.destroy$)
      ...
      this._store.pipe(select((s: ICoreState) => s.ui),
        takeUntil(this.destroy$)
