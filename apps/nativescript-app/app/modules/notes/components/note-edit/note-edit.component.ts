...
import { ProgressIndicatorActions, UserActions, UserState, ProgressService, WindowService, LogService, BaseComponent, ModalActions, ICoreState, UIState, ModalState } from '@ngatl/core';
...
import { ViewPhotoComponent } from '@ngatl/nativescript/features/ui/components/view-photo/view-photo.component';
...
export class NoteEditComponent extends BaseComponent {//BaseModalComponent {
  ...
  ngOnInit() {
    ...
    this.recordService.transcription$.pipe(
      takeUntil(this.destroy$)
     ) .subscribe((text) => {
        this.item.note = text;
      });
