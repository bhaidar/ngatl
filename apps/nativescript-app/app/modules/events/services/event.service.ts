...
import { Cache, StorageKeys, StorageService, UserService, UserState, LogService, WindowService, environment } from '@ngatl/core';
import * as localNotifications from "nativescript-local-notifications";
import { RouterExtensions } from 'nativescript-angular/router';

// app
import { IConferenceAppState, IConferenceState } from '../../ngrx';
import { sortAlpha, dateIsValid } from '@ngatl/utils';
import { EventActions } from '../actions';
import { ConferenceViewModel, Session } from '../models/conference.model';
import { EventState } from '../states';
import { AWSService, AppService } from '@ngatl/nativescript';
...
export class EventService extends Cache {
  ...
  public fetch(forceRefresh?: boolean): Observable<Array<Session>> {
    ...
    if (!forceRefresh && stored) {
      ...
      return of(this._createSessions(stored));
    } else {
      // console.log('fetch events fresh!');
      // return Observable.create(observer => {
      //   this.aws.getJSON(`https://s3.amazonaws.com/ng-atl/ConferenceEvents.json`).then(events => {
        return this.http.get(`${environment.API_URL}ConferenceEvents`) //?filter%5Bwhere%5D%5Btype%5D=Workshop
          .pipe(map((events: Array<EventState.IEvent>) => {
