...
export class StorageService {
  ...
  constructor(public win: WindowService) {
    this.storageType =
      this.win && this.win['localStorage'] ? this.win['localStorage'] : null;
  }
  ...
  public setItem(key: string, value: any): void {
    if (key) {
      ...
      console.log('storage.setItem problem, key is null');
    }
  }
  ...
  public getItem(key: string): any {
    if (key) {
      ...
      console.log('storage.getItem problem, key is null');
    }
    return undefined;
  }
  ...
  public removeItem(key: string): void {
    if (key) {
      ...
      console.log('storage.removeItem problem, key is null');
    }
  }
...
export class Cache implements ICache {
   ...
   */
  public clear(): void {
    const keys = this.keys || (this.key ? [this.key] : []);
        ...
        if (c && Array.isArray(c)) {
          ...
          for (let i = 0; i < c.length; i++) {
            if (typeof c[i] !== 'object') {
              console.error(
                `Cache: invalid value (not an object) in collection.`
              );
              console.error(c[i]);
              return;
            } else {
